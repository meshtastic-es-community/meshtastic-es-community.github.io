...
:::warning
La mayor parte de España **no usa un _preset_ de fábrica**, sino la configuración personalizada de la comunidad
(**SFNarrow**), que no se puede fijar desde `userPrefs.jsonc` (ver [limitaciones](#limitaciones)). El `LONG_FAST` del
ejemplo es solo un valor de partida: tendrás que ajustar el ancho de banda, el _spreading factor_ y la frecuencia
después, siguiendo la [configuración inicial](../guias-basicas/configuracion-inicial.mdx#region-preset).
:::
...
| `USERPREFS_CONFIG_OWNER_SHORT_NAME` | Nombre corto / acrónimo, máximo 4 caracteres (ej. `"NCM1"`).         |
...
| Lite Fast _(EU 868MHz)_   | `meshtastic_Config_LoRaConfig_ModemPreset_LITE_FAST`     |
| Lite Slow _(EU 868MHz)_   | `meshtastic_Config_LoRaConfig_ModemPreset_LITE_SLOW`     |
